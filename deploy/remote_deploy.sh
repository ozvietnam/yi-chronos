...
docker exec yi-chronos sh -c '
  ...
  echo "  seeded tu_vi json: $(ls data/tu_vi/*.json 2>/dev/null | wc -l) | seeds: $(ls data/seeds/ 2>/dev/null | wc -l) | q1 dicts: $(ls data/yi_publishing/q1_tuvi/master/*.json 2>/dev/null | wc -l) | than_so master: $(ls data/than_so/master/*.json 2>/dev/null | wc -l)"
  ls data/than_so/master/balliett_tone_color.json data/than_so/master/cheiro_birth_numbers.json embedded_data/than_so/master/balliett_tone_color.json 2>&1 | sed "s|^|  than_so check: |"
